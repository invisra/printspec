...
test('static schema site files exist',()=>{
 for(const file of ['public/index.html','public/printspec/index.html',`public/printspec/${version}/index.html`,'public/printspec/manifest.json',`public/printspec/${version}/manifest.json`]){
...
test('manifests include versions and all schemas',()=>{
 const project=JSON.parse(fs.readFileSync(path.join(root,'public/printspec/manifest.json'),'utf8'));
 assert.equal(project.project,'printspec');
 assert.deepEqual(project.versions.map((v)=>v.version),[version]);
 const manifest=JSON.parse(fs.readFileSync(path.join(publicDir,'manifest.json'),'utf8'));
 assert.equal(manifest.version,version);
 assert.deepEqual(manifest.schemas.map((s)=>s.filename),schemaFiles);
