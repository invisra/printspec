...
@pytest.mark.skipif(
    not ts_cli.exists(), reason="TypeScript package not built; run `npm run build` first"
)
...
def test_python_and_typescript_generator_support_agree(filename, tmp_path):
    ...
    ts_scad = subprocess.run(
        ["node", str(ts_cli), "to-openscad", str(spec_path)],
        capture_output=True,
        text=True,
        timeout=20,
    )
    ts_cq = subprocess.run(
        ["node", str(ts_cli), "to-cadquery", str(spec_path)],
        capture_output=True,
        text=True,
        timeout=20,
    )
