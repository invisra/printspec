...
def _part_family_entries():
    ...
    for filename, schema in SCHEMAS.items():
        ...
        if isinstance(part_type, str) and schema.get("properties", {}).get("parameters"):
            entries.append((part_type, filename, schema))
    ...
    return [
        {
            ...
            "schemaFilename": f,
            "generatorSupported": t in _GENERATOR_SUPPORTED,
        }
        for t, f, s in _part_family_entries()
    ]
...
def get_part_family_form_metadata(part_type: str) -> dict:
    ...
    if entry is None:
        raise ValueError(f"Unsupported printspec part family: {part_type}")
    _, _, schema = entry
    ...
    if isinstance(ui.get("order"), list):
        ordered = [n for n in ui["order"] if n in known]
    else:
        ...
        groups = [
            {
                ...
                "fields": [n for n in g.get("fields", []) if n in known],
            }
            for g in ui["groups"]
        ]
    else:
        groups = [
            {"id": "parameters", "title": parameters.get("title", "Parameters"), "fields": ordered}
        ]
    fields = []
    for name in ordered:
        ...
        for out, key in [
            ("control", "x-printspec-control"),
            ("unit", "x-printspec-unit"),
            ("step", "x-printspec-step"),
            ("minimum", "minimum"),
            ("exclusiveMinimum", "exclusiveMinimum"),
            ("maximum", "maximum"),
            ("default", "default"),
            ("examples", "examples"),
            ("priority", "x-printspec-priority"),
        ]:
            if key in field:
                item[out] = field[key]
        fields.append(item)
    return {
        "partType": part_type,
        "title": schema.get("title", part_type),
