...
function main(argv=process.argv.slice(2)){
 ...
 if(!['validate','to-openscad','to-cadquery','bom','bundle'].includes(cmd)) return fail(`error: unknown command ${cmd}`);
 ...
 if(cmd==='bundle'){const out=opt(rest,'--output'); if(!out) return fail('error: bundle requires --output <directory>'); const b=createBundle(spec,{includeOpenScad:!rest.includes('--no-openscad'),includeCadQuery:!rest.includes('--no-cadquery'),includeBom:!rest.includes('--no-bom'),includePartCad:rest.includes('--partcad')}); if(!b.supported) return fail(`error: ${b.message??'unsupported bundle'}`); try{writeBundleToDirectory(b,out,{overwrite:rest.includes('--overwrite')});}catch(e:any){return fail(`error: ${e.message}`)} console.log(`wrote ${b.files.length} files to ${out}`); console.log(`warnings: ${b.warnings.length}`); return 0;}
