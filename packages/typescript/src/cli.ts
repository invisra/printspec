...
function load(file:string){try{return JSON.parse(fs.readFileSync(file,'utf8'));}catch(e:any){throw new Error(`${file}: parse error: ${e.message}`);}}
