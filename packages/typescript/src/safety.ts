...
export const isPotentiallyUnsafeLabel = (text: string) =>
  bad.some((w) => text.toLowerCase().includes(w));
...
export function validateSafeMetadata(spec: any) {
  ...
  return {
    ...
    errors: isPotentiallyUnsafeLabel(text)
      ? ["Metadata or label may describe a disallowed/safety-critical use."]
      : [],
  };
}
