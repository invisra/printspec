...
function findSchemaDir(): string {
  ...
  throw new Error(
    "Unable to locate bundled printspec schemas. Run npm run sync:schemas before building, or reinstall the package.",
  );
}

export const schemaDir = findSchemaDir();
export const schemaFiles = fs
...
export function createAjv() {
  return createAjvFromSchemas(schemas);
}
