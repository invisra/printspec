...
function readme(
  ...
  const lines: string[] = [];
  lines.push(
    `# ${kind === "project" ? (spec.project?.label ?? "printspec project") : (spec.part?.label ?? "printspec part")}`,
    "",
  );
  if (kind === "part") lines.push(`Part type: ${spec.part?.type ?? "unknown"}`);
  else {
  ...
  lines.push(
    `printspec version: ${spec.printspecVersion ?? VERSION}`,
    "",
    "## Generated files",
    ...files.map((f) => `- ${f.path} (${f.role})`),
    "",
  );
  lines.push(
    "## BOM",
  ...
  lines.push("Generated CAD source should be reviewed before manufacturing.");
  return lines.join("\n") + "\n";
}
function partcad(spec: any, files: RoleFile[]) {
  const cq = files
    .filter((f) => f.role === "cadquery-source")
    .sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
  const name = spec.project?.partcad?.packageName ?? "printspec-project";
  const lines = [
    "package:",
    `  name: ${name}`,
    `  version: ${spec.project?.partcad?.packageVersion ?? VERSION}`,
  ...
  return j(
    {
      ...
      createdBy: "printspec",
      printspecVersion: spec.printspecVersion ?? VERSION,
      kind,
      files: entries,
      warnings,
    },
...
export function createBundleWithDeps(
  deps: BundleDeps,
  input: unknown,
  options: BundleOptions = {},
): BundleResult {
  const r = deps.validatePrintSpec(input as any);
  if (!r.valid)
    return {
      supported: false,
      files: [],
  ...
  const spec = normalizePrintSpec(input as PrintSpec);
  const opts = {
    includeOpenScad: options.includeOpenScad !== false,
    includeCadQuery: options.includeCadQuery !== false,
    includeBrepJs: options.includeBrepJs === true,
    includeBom: options.includeBom !== false,
    includePartCad: options.includePartCad === true,
  ...
  if (kind === "part") {
    ...
    for (const p of spec.project!.parts ?? []) {
      ...
      if (p.spec) {
        ...
        add(
          files,
          `parts/${id}/printspec.json`,
          j(ps, opts.prettyJson),
          jsonMedia,
          "part-source-spec",
        );
        ...
        warn(
          warnings,
          `External specPath references are not bundled yet: ${p.specPath}`,
          `parts/${id}/printspec.json`,
        );
    }
    if (opts.includeBom) bomFiles(files, "bom/", extractBom(spec));
  ...
  add(
    ...
    manifest(kind, files, warnings, spec),
    jsonMedia,
    "bundle-manifest",
  );
