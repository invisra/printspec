...
const generatorSupported = new Set([
...
export function getPartFamilyFormMetadata(partType: string): FormMetadata {
  ...
  const ordered = Array.isArray(metadata.order)
    ? metadata.order.filter((name: string) => known.has(name))
    : Object.keys(properties).sort();
  for (const name of Object.keys(properties).sort())
    if (!ordered.includes(name)) ordered.push(name);
  const groups: FormGroup[] =
    Array.isArray(metadata.groups) && metadata.groups.length
