...
export function resolveCableClipGeometry(q: Params) {
  const baseLength = num(q.baseLength, 28), baseWidth = num(q.baseWidth, num(q.width, 12)), baseThickness = num(q.baseThickness, num(q.thickness, 3));
  ...
  return {baseLength, baseWidth, baseThickness, innerDiameter, wallThickness, openingWidth, outerDiameter, centerZ};
}
