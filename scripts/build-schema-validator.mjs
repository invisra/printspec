...
async function tryEsbuild() {
  try {
    ...
    await esbuild.build({
      ...
      minify: true,
      alias: {
        "@invisra/printspec/browser": path.join(
          root,
          "packages/typescript/src/browser.ts",
        ),
      },
    });
    return true;
    ...
    console.warn(
      `esbuild unavailable; using schema-site fallback bundler (${error.message})`,
    );
    return false;
  }
}
...
async function fallbackBundle() {
  const schemas = JSON.parse(
    readFileSync(
      path.join(root, "packages/typescript/src/generated/schemas.generated.ts"),
      "utf8",
  ...
  try {
    ...
    source = ts.transpileModule(sourceTs, {
      compilerOptions: {
...
for (const pattern of forbidden)
  if (pattern.test(bundled))
    throw new Error(
      `Validator bundle contains forbidden Node built-in pattern: ${pattern}`,
    );
