...
async function tryEsbuild() {
  try {
    ...
    await esbuild.build({
      ...
      minify: true,
      alias: {
        "@invisra/printspec/browser": path.join(root, "packages/typescript/src/browser.ts"),
      },
    });
    return true;
  } catch (error) {
...
for (const pattern of forbidden)
  if (pattern.test(bundled))
    throw new Error(`Validator bundle contains forbidden Node built-in pattern: ${pattern}`);
