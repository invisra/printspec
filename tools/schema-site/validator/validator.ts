...
} from "@invisra/printspec/browser";
...
function validateCurrent() {
  ...
  if (result.valid) {
    ...
    setResult(
      "success",
      `<p class="status">Valid PrintSpec JSON</p><dl><dt>Part type</dt><dd>${escapeHtml(part.type)}</dd><dt>Label</dt><dd>${escapeHtml(part.label)}</dd><dt>Units</dt><dd>${escapeHtml(spec.units)}</dd></dl>`,
    );
    renderMessages(
      warnings.length ? warnings : ["No warnings."],
      warnings.length ? "warning" : "warning",
    );
  } else {
    setResult(
      "error",
...
$("clearButton").addEventListener("click", () => {
  input.value = "";
  exampleSelect.value = "";
  setResult("neutral", "Load an example or paste JSON, then click Validate.");
  renderMessages([]);
});
